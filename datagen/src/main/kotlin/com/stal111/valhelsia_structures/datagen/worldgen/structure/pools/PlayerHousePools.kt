...
object PlayerHousePools : RegistryDataProvider<StructureTemplatePool> {
    ...
    override fun bootstrap(context: BootstrapContext<StructureTemplatePool>) {
        BuiltInStructurePools.PLAYER_HOUSES.create(
            HELPER,
            context,
            "player_house",
            UnaryOperator { it.element("house_1").element("house_2") })
        HELPER.create(
            "player_house/feature_plate",
            context,
            "player_house",
            UnaryOperator {
        ...
        HELPER.create(
            ...
            "player_house",
            UnaryOperator {
                it.element("feature_farm_1").element("feature_farm_2").element("feature_farm_3")
                    .element("feature_bee").element("feature_chicken")
            })
        HELPER.create(
            "player_house/portals_and_farms",
            context,
            "player_house",
            UnaryOperator {
                it.element("feature_farm_1").element("feature_farm_2").element("feature_farm_3")
                    .element("feature_bee").element("feature_chicken").element("feature_portal_1")
                    .element("feature_portal_2").element("feature_portal_3")
            })
    }
}