...
object DesertHousePools : RegistryDataProvider<StructureTemplatePool> {
    ...
    override fun bootstrap(context: BootstrapContext<StructureTemplatePool>) {
        BuiltInStructurePools.DESERT_HOUSES.create(
            HELPER,
            context,
            "desert_house",
            UnaryOperator { it.element("desert_house") })
        HELPER.create(
            "desert_house/oasis_plate",
            context,
            "desert_house",
            UnaryOperator { it.element("oasis_plate") })
        HELPER.create(
            "desert_house/well_or_oasis_plate",
            context,
            "desert_house",
            UnaryOperator { it.element("well_or_oasis_plate") })
        HELPER.create(
            "desert_house/oasis",
            context,
            "desert_house",
        ...
        HELPER.create(
            "desert_house/wells_and_oasis",
            context,
            "desert_house",
            UnaryOperator {
                it.element("feature_oasis_1").element("feature_oasis_2").element("feature_well_1")
                    .element("feature_well_2")
            })
    }
}